...
static inline void FFT32_forward(FFT32 *self, Float32 *__restrict inputFrame)
{
    ...
    //    self->splitComplex.realp[self->FFTFrameSizeOver2 - 1] = self->splitComplex.imagp[0];
    //    self->splitComplex.imagp[self->FFTFrameSizeOver2 - 1] = 0.0f;
...
static inline void FFT64_forward(FFT64 *self, Float64 *__restrict inputFrame)
{
    ...
    self->splitComplex.realp[self->FFTFrameSizeOver2] = self->splitComplex.imagp[0];
    self->splitComplex.imagp[self->FFTFrameSizeOver2] = 0.0f;
    self->splitComplex.imagp[0] = 0.0f;
}
